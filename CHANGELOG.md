...
### Added
- **MSCS DS V5.2 — sistema de dois modos (2026-08-05)** — `DESIGN_SYSTEM.md` na raiz entra como spec do sistema Tradutor (papel com contorno) / Veredicto (preto elétrico), consolidando V3.1, V3.2, V4.0, V5.0 e V5.1 daquela linha. Implementação em `css/ms-v5.css` (tokens `--ms-*`, escala tipográfica §6, contorno de 3px, sombra tingida, termo-chave em guilhemés) e `js/rabisco.js` (gerador determinístico de textura, semente fixa `20260805`, zona de texto rejeitada no gerador). `tools/contrast.mjs` lê os hex direto do CSS e falha se algum par divergir do documentado ou reprovar no piso WCAG do seu papel — 21 pares conferem. **Camada paralela:** prefixo `--ms-*` não colide com `--ds-*`/`--p-*` da Era Midnight, e nenhuma superfície pública (`index.html`, `products.html`, `formats.html`, `governance.html`, `studies.html`) muda de aparência. Qual dos dois sistemas governa o site segue em aberto (`DESIGN_SYSTEM.md` §10.4).
- **Spec textual canônica in-repo (2026-06-22)** — `docs/DESIGN.md` (v3.2 portável, auto-contido, Claude Design ready) + `docs/PROCEDURAL_DESIGN_CODEX_PIPELINE.md` (método de 6 passos do Design Codex™) versionados neste repo. Reverte a parte de "fora do repo" da DEC-018/commit `114e4de`: o pointer anterior apontava pra `MD FILES/KNOWLEDGE/` (gitignored), deixando o entregável do item #10 LEARN sem backup em git. `KNOWLEDGE/` segue como working copy de upload, espelhada daqui. `docs/CANONICAL_REFERENCE.md` repontado.

### Fixed
- **Cobalto do Veredicto (2026-08-05)** — a tabela de função de cor do `DESIGN_SYSTEM.md` §4 citava o cobalto em `#3A3FA8`, o valor reprovado na calibragem V5.1 (2.19:1 sobre a superfície do modo, abaixo do piso de 3.0), enquanto o token do §3 já trazia o `#4B53E0` corrigido. Prevalece o token; a tabela passa a citar `#4B53E0`. `tools/contrast.mjs` cobre o par para a regressão não voltar em silêncio. Declarada também a base de medição das razões WCAG do documento: `--ms-surface` do modo, não `--ms-bg`.

### Changed
- **DEC-024 (2026-06-17)** — Fraunces unificado. Fraunces passa a governar headline e ênfase nos dois registros (produto E editorial) via eixo óptico `opsz`. Typo B (IBM Plex Sans + Libre Caslon Text + Space Mono) retirado: `--ds-ed-font-headline`/`-emphasis` → Fraunces (ênfase em italic), `--ds-ed-font-body` → Inter Tight (corpo universal), `--ds-ed-font-label` → IBM Plex Mono (único monoespaçado). Legados comentados em `tokens.css` (1 ciclo, DEC-017). Registro produto = opsz 144 / wght 700–900 (impacto); editorial = opsz 24–36 / wght 340–400 (quieto). Propagado em `tokens.css`, `story-tokens.css`, `editorial.css`, `templates/story.html`, `index.html`, `products.html`, `governance.html` (matriz de roteamento reescrita: registro produto vs editorial da mesma Fraunces) e docs. Cormorant (Typo C) intocada, reservada a manifesto/longform. Caixa intocada (DEC-023). Cor intocada (DEC-020/021/022). Estudos congelados em `studies/` e mockups embutidos (OFFF/`.offf-story-mockup`, `studies.html`, `formats.html`) preservados como registro histórico na era Typo B.
